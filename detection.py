...
#         for slice_number in range(1):
        ...
        if args["noise_fn"] == "gauss":
            ...
            timestep = random.randint(int(args["sample_distance"] * 0.3), int(args["sample_distance"] * 0.6))
...
#                         img_mask[0, ...].reshape(1, 3, *args["img_size"]),
#                         output_name + ".png"
#                         )
#                 )

#         plt.close('all')
        print(f"{i} out of {len(ano_dataset)} ")
        if args["noise_fn"] == "gauss":
...
#                     img_mask[0, ...].reshape(1, 1, *args["img_size"]), denoise_fn=args["noise_fn"],
#                     total_avg=3
#                     )
#             dice_data.append(dice)
...
#                     img_mask[0, ...].reshape(1, 1, *args["img_size"])
#                     )
#             dice = diff.detection_B(
#                     unet, img[slice_number, ...].reshape(1, 1, *args["img_size"]),
#                     args, (new["filenames"][0][-9:-4], new["slices"][slice_number].numpy()[0]),
#                     img_mask[0, ...].reshape(1, 1, *args["img_size"]), "octave",
#                     total_avg=3
#                     )
#             dice_data.append(dice)
...
#         * 4
    loader = iter(torch.utils.data.DataLoader(d_set, batch_size=args['Batch_Size'], shuffle=False))
#             )
# dataset.init_dataset_loader(d_set, args)
    plt.rcParams['figure.dpi'] = 200
...
#         mse = (mse > 0.5).float()
#         print("mse", mse.shape)
        mask= torch.tensor(cv2.threshold(mask.cpu().numpy(), 0, 1, cv2.THRESH_BINARY)[1]).reshape(1, *mask.shape)
...
#             )
    ...
    try:
        ...
        pass
    t_range = np.linspace(0, 50, 1000).astype(np.int32)
    ...
    #         mse = (mse > 0.5).float()
    #         print("mse", mse.shape)
            img_mask= torch.tensor(cv2.threshold(mask.cpu().numpy(), 0, 1, cv2.THRESH_BINARY)[1]).reshape(1, *mask.shape)
    #         print("mask", mask.shape)
            dice_data.append(evaluation.dice_coeff(img, output.to(device), img_mask, mse=mse).cpu().item())
            ssim_data.append(
                    evaluation.SSIM(
                            img.permute(0, 2, 3, 1).reshape(*args["img_size"], img.shape[1]),
                            output.permute(0, 2, 3, 1).reshape(*args["img_size"], img.shape[1])
                            )
                    )
